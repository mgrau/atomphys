...
class Isotope(dict):
    ...
    def __init__(self, USE_UNITS=False, ureg=None, **isotope):
        ...
        if (
            "Atomic number" in isotope
            and isotope["Atomic number"] is not None
            and len(isotope["Atomic number"]) > 0
        ):
            try:
                atomic_number = int(
                    re.search("\\d+", isotope["Atomic number"]).group()
                )
        ...
        if (
            "Neutron number" in isotope
            and isotope["Neutron number"] is not None
            and len(isotope["Neutron number"]) > 0
        ):
            try:
                neutron_number = int(
                    re.search(
                        "\\d+", isotope["Neutron number"]).group()
                )
            except AttributeError:
        ...
        if (
            "Atomic mass" in isotope
            and isotope["Atomic mass"] is not None
            and len(isotope["Atomic mass"]) > 0
        ):
            try:
                atomic_mass = float(
                    re.search(
                        "\\d+\\.*\\d*", isotope["Atomic mass"]).group()
                )
            except AttributeError:
        ...
        if "Spin (physics)" in isotope and len(isotope["Spin (physics)"]) > 0:
            try:
                spin = Fraction(
                    re.search("\\d+[/]*\\d*",
                              isotope["Spin (physics)"]).group()
                )
            except AttributeError:
        ...
        if "Natural abundance" in isotope and len(
                isotope["Natural abundance"]) > 0:
            try:
                abundance = float(
                    re.search(
                        "^\\d+\\.\\d+", isotope["Natural abundance"]).group()
                )
            except AttributeError:
        ...
        if "mag_moment_μN" in isotope and isotope["mag_moment_μN"] is not None:
            try:
                value = float(
                    re.search(
                        "\\d+\\.+\\d+", isotope["mag_moment_μN"]).group()
                )
            except AttributeError:
            ...
            if self._USE_UNITS:
                try:
                    magnetic_moment = self._ureg.Quantity(
