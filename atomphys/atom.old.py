...
class Atom:
    ...
    def scalar_polarizability(self, state, omega):
        ...
        RME2 = 3*c_.pi*c_.epsilon_0*c_.c**3/omega0**3 * \
            (2*t_up.J_k+1)/(2*t_up.J_i+1)*t_up.gamma
    ...
    def plot_states(self, num_states=0, min_spacing=0.0, min_gamma=1e3,
        ...
        while any(np.diff(energy) < min_spacing):
            ...
            energy[fix_index+1:] += min_spacing*1.01 - \
                np.diff(energy[fix_index:fix_index+2])
        energy *= max_energy
        ...
        if show_transitions:
            for i in range(len(states)):
                ...
                if transitions.empty == False:
                    transitions = transitions[[E_k in np.array(
                        states.E) for E_k in transitions.E_k]]

                if transitions.empty == False:
                    angle = np.arctan2(
                        transitions.E_k - transitions.E_i, transitions.L_k - transitions.L_i)
                    transitions = transitions.iloc[np.argsort(angle)[::-1]]
                    xpos = np.linspace(
                        state.L-line_width/3, state.L+line_width/3, len(transitions)+2)[1:-1]
                    for k in range(len(transitions)):
                        ...
                        angle = (180/np.pi) * \
                            np.arctan2(s1[1]-s0[1], s1[0]-s0[0])
                        if angle > 90:
                            angle -= 180
                        plt.annotate('',
                        ...
                        label = '$\\lambda = %s m$\n' % to_SI(transition.wl)
                        label += '$\\Gamma = 2\\pi \\times %sHz$' % to_SI(
                            transition.gamma/(2*np.pi))
    ...
    def get_states(self):
        s_i = self.transitions[['E_i', 'n_i',
                                'S_i', 'L_i', 'J_i']].drop_duplicates()
        s_i.columns = ['E', 'n', 'S', 'L', 'J']
        s_k = self.transitions[['E_k', 'n_k',
                                'S_k', 'L_k', 'J_k']].drop_duplicates()
    ...
    def _find_conditions(self, search_string, match_pattern):
        ...
        * 1<S will be converted to [1], ['<'], S
        ...
        if len(matches) == 2:
            ...
            # the thing between the two conditions
            term = str(search_string[matches[0][1]:matches[1][0]])
    ...
    def select_states(self, condition):
        ...
        * `selected_data` -- The state data in self.states that satisfies `condition`
        ...
        for cond in condition:
            ...
            n_l, term_l, operators_l = self._find_conditions(
                cond, '<={0,1}')  # matches < and <=
            n_g, term_g, operators_g = self._find_conditions(
                cond, '>={0,1}')  # matches > and >=
            ...
            # we couldn't find a <, <=, >, >= condition. Hence it must be an == condition.
            if term is None:
                # We don't want to call this when there are also <= and >= signs since the matching pattern will also match those.
                # matches = and == but also <= and >=.
                n, term, operators = self._find_conditions(cond, '={1,2}')

            for (num, op) in zip(n, operators):
                if op == '<':
                    selected_data = selected_data.loc[num <
                                                      selected_data[term]]
                elif op == '<=':
                    selected_data = selected_data.loc[num <=
                                                      selected_data[term]]
